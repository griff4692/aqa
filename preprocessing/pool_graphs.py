...
def resolve_node_edge(new_node1_str, new_node2_str, edge_str, graph, tf_idf) -> Node:
    match1 = _node_match(new_node1_str, graph, tf_idf)
    match2 = _node_match(new_node2_str, graph, tf_idf)
...
def _node_match(new_node_str, graph, tf_idf):
    match = Node(name=new_node_str, weight=1)
    max_sim = 0.0
    for node in graph.nodes():
        max_overlap = max(list(map(lambda x: overlap(new_node_str, x, tf_idf), node.all_names())))
...
def build_graph(item, graph, tf_idf):
    for ie_tup in item[-1]['ie_tups']:
        node_1, edge, node_2 = ie_tup
        resolve_node_edge(node_1, node_2, edge, graph, tf_idf)
...
if __name__ == '__main__':
    ...
    start_time = time()
    graph = Graph()
    items = list(contexts_ie_debug.items())
